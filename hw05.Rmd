...
In this assignment, I will be working with the `gapminder` dataset to (1) Reorder a factor, (2) Write some data to file and load it back into R, (3) Improve a figure using new knowledge, (4) Make a plotly visual and (5) Implement visualization design principles.
...
> Experiment with one or more of write_csv()/read_csv() (and/or TSV friends), saveRDS()/readRDS(), dput()/dget() and explore whether this survives the round trip of writing to file then reading back in.

Here, we will try to expirent with downloading and uploading the dataframe we just created `gapminder_Europe_sorted` using three different file types.
...
> Remake at least one figure or create a new one, in light of something you learned in the recent class meetings about visualization design and color. Reflect on the differences. Then, make a new graph by converting this visual to a plotly graph.
...
1. Created a new variable for the final plot.  
...
Next, I wanted do this plot for all other continents with countries whose `gdpPercap` is between $5,000 and $1,0000.
...
We can notice here one of the continents doesn't have data that satisfies this requirement (Oceania). There is also no longer a linear relationship between `gdpPercap` and `lifeExp` - this pattern holds true for all four continents.
...
Similarly, let's turn `gapminder_gdpPercap` into the flotly file.
...
However, if we can keep the `year` variable as a frame, we no longer have to restrict our observations to a small number of countries. We will exclude Oceania as it only has 2 countries.
...
```{r}
ggsave("gapminder_Euro.png", plot = gapminder_Europe_gdpPercap, device="png")
```

The `gapminder_Europe_gdpPercap` plot can be found [here](--).

```{r}
ggsave("gapminder_gdpPercap.png", plot = gapminder_gdpPercap, device="png")
```

And the `gapminder_gdpPercap` plot image is located [here](--).